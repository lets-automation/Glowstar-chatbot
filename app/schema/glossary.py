...
DATA_NOTES = [
    ...
    "step, so a dept's labour can end (e.g. Fency's last ProcessDate is 2026-05-30) while its "
    ...
    "GIA RESULTS 'EMPLOYEE-WISE' IS AMBIGUOUS — ASK which employee role (this is a real "
    "client pain point). A GIA-graded packet (tblFinalPacket, Lab='GIA') touches THREE "
    "different people, giving THREE different lists: (1) the UPLOAD / data-entry clerk = "
    "tblFinalPacket.UserID — this is 'who handled the certificate upload', and it is often "
    "ONE person who entered EVERY packet, so grouping by it returns a single name (NOT what "
    "a manager usually wants); (2) the MAKER / POLISHER of record = tblPctChecker "
    "(MfgEmpId / PolishEmpId) JOIN tblEmployee — these are the MFG-department karigars; "
    "(3) the per-STAGE worker, including the 'Fency' (fancy-shape) stage = tblPointRateLabour "
    "(Emp_ID, DepartmentName), joined to the GIA result by Packet_ID = tblFinalPacket.PacketID. "
    "For 'GIA results for the Fency department, employee-wise' the user almost never means the "
    "upload clerk — they mean the FENCY WORKER (option 3) or the maker (option 2). Because the "
    "roles diverge, ASK a short numbered question (upload clerk / maker-polisher / Fency-stage "
    "worker) OR answer and DECLARE the role you used. The department is spelled 'Fency' (dept "
    "code Y), reachable via tblPointRateLabour.DepartmentName='Fency' or tblEmployee."
    "DepartMentName='Fency'; it does NOT appear as the maker on tblFinalPacket/tblPctChecker.",
...
JOIN_HINTS = [
    ...
    "AND the breakdown by type, e.g. 'June 1–26: 132 damage records — 67 DAMAGE "
